...
import com.suibianshuo.util.common.MD5Util;
import com.suibianshuo.util.wechat.WXPayConstants.SignType;
...
private static Logger log = LoggerFactory.getLogger(WXPayUtil.class);
	...
	public static Map<String,Object> createOrderInfo(Map<String, String> requestMap) {  
	    ...
	    try {
			unifiedOrderRequest.setSign(generateSignature(packageParams,WeiXinConfig.API_KEY));//签名
		} catch (Exception e) {
			e.printStackTrace();
		}
	    //将订单对象转为xml格式  
...
//    });
	...
	public static Map<String, String> httpOrder(String orderInfo) {  
	    ...
	    try {  
	        HttpURLConnection conn = (HttpURLConnection) new URL(WeiXinConfig.UNIFIED_ORDER_URL).openConnection();  
	        //加入数据    
